...
function link(text,href){const n=el('a','certificate-view',text);n.href=href;n.target='_blank';n.rel='noopener noreferrer';return n;}
...
class AwsCertifications extends HTMLElement{
 connectedCallback(){
  ...
  const panel=el('div','certificate-modal-panel'),close=el('button','certificate-close','Close ×'),title=el('h3'),metadata=el('div'),image=el('img','certificate-full'),error=el('p','','The certificate could not be loaded. Please try again later.'),actions=el('div','certificate-actions');
  ...
  for(const cert of certifications){
   ...
   const buttons=()=>{const group=el('div','certificate-actions'),view=link('View Certificate',file.pdf&&certificateFile(cert.thumbnail)?cert.thumbnail:file.path);view.addEventListener('click',e=>{if(file.pdf&&!certificateFile(cert.thumbnail))return;e.preventDefault();e.stopPropagation();opener=view;title.textContent=cert.name;metadata.replaceChildren(details(cert));error.hidden=true;image.hidden=false;image.src=file.pdf?cert.thumbnail:file.path;image.alt=cert.name+' — certificate preview';actions.replaceChildren();if(verify)actions.append(link('Verify Credential',verify));previousOverflow=document.body.style.overflow;dialog.showModal();document.body.style.overflow='hidden';close.focus();});group.append(view);if(verify)group.append(link('Verify Credential',verify));return group;};
